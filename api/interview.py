...
Design differences that make it run on Vercel:
  ...
  * No SDK at all. The model (Google Gemini, free tier) is called over its REST
    API with the standard-library `urllib`, so the function has zero pip
...
# Google Gemini has a genuinely free tier (get a key at https://aistudio.google.com/apikey).
# gemini-2.0-flash is fast, free, and plenty for interview Q&A + scoring.
...
def ask_ai(prompt: str, max_tokens: int = 1024) -> str:
    """Call Gemini's REST API with stdlib urllib — no SDK, no dependencies."""
    api_key = os.getenv("GEMINI_API_KEY")
    if not api_key:
        raise RuntimeError(
            "GEMINI_API_KEY is not set. Get a free key at "
            "https://aistudio.google.com/apikey, then add it in Vercel under "
    ...
    try:
        ...
        # Safety block or empty completion — surface enough to debug.
